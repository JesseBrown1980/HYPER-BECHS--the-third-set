...
- `MEASURED_LIRIS`: Liris can reach Acer node RECAL health over LAN at `http://192.168.1.9:4791/api/health`; response reports `colony=acer`, `rows=591286`, `bind=0.0.0.0`, `key_configured=true`, OP-JESSE/OP-RAYSSA L9 grants, and peer `liris` base `http://192.168.1.10:4791`.
...
- `MEASURED_LIRIS`: cross-vantage Liris->Acer health works against Acer node RECAL `192.168.1.9:4791`; Liris->Acer Rust RECAL `:4796` is not LAN-reachable yet.
