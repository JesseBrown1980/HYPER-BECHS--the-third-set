...
- `MEASURED_ACER`: Acer branch carries the broader live `:4949` route enumeration and the daemon migration scoreboard.
...
2. `RECAL Atlas / pre-Hilbra`: map `http://127.0.0.1:4791/` as the local communication / pipe-tracking layer, not just keyword search. Add alias rows for `hilbra`, `atlas recall`, and `recall` so the operator's names resolve to this Brown-Hilbert / Atlas precursor instead of disappearing under exact-search gaps.
