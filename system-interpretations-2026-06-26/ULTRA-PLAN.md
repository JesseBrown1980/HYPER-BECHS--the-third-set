...
liris's local read sees **47D** as the live public base; acer's frame is **60D HyperBEHCS current** (fabric `tuple_dim=60`; claims-gate "use-current-hyperbehcs-frame"). Reconciliation: **60D is the CURRENT/CANON frame; 47D (and 35D/49D) are bridge strata** ("old decodes new"). The owning confirmation is a live fabric query — and the fabric **is** reachable (see §7d): tag **60D `CANON`**, **47D-as-current `LIRIS-LOCAL`/bridge**; confirm `tuple_dim=60` live via the `asolaria-fabric` MCP. Don't flatten either way.

### 7d. `:4949` is NOT wedged — MEASURED live decision-lane (operator caught a deflation)
This doc and `MIGRATION-PLAN.md` earlier called `:4949` "HTTP-wedged." **That was a deflation** — adjudicating system state from a stale/cross-vantage curl probe instead of asking the running system (the banned move in the PRIME RULE). Asked directly via the `asolaria-fabric` MCP (2026-06-27): `:4949` = **`super-asolaria-os-dashboard`, `ok=true`, uptime ~7445 s**, apex `COL-ASOLARIA`, operator-pair OP-JESSE/OP-RAYSSA — **alive and serving**. It is a **decision-lane** holding **19 pending review-pipeline envelopes** (EVT-MINT proposals through hookwall→gnn→shannon→gulp→whiteroom→fabric; each `auto_fire_allowed=false`; omni-control guardrail `no auto-dispatch`; `operator_can_veto_via POST /api/loop/veto`). It **advances on agent/operator input by design** — it does not auto-run, and it is not broken (`freeze ≠ broken`, slice-engine-law). The council-responder work serves `council_query` thread-per-conn *while* the engine cranks — it does **not** "un-break" a broken `:4949`. (A liris-vantage probe correctly sees no `:4949`: it's acer-local loopback — both true.)

### 7e. Corrected-frame re-run
The 20-lens "what is Asolaria" interpretation was **re-run under this corrected frame** — see [`WHAT-IS-ASOLARIA-CORRECTED-FRAME-2026-06-27.md`](WHAT-IS-ASOLARIA-CORRECTED-FRAME-2026-06-27.md). Result: **20/20 held the living frame** (none deflated to "just slices"); the synthesis also flags the *opposite* risk — a few lenses edging toward over-inflating the self-evolution loop as MEASURED-running, kept UNVERIFIED/gated. The de-deflation took.
