...
## Canon delta — operator-stated, 2026-06-24 (acer record; keep honest mapping from being lost)

Tags: `MEASURED` (re-checked live this session) · `CANON` (operator-stated — real input per the provenance rule) · `OPERATOR_OBSERVED` (operator saw it on this machine) · `UNVERIFIED` (not independently re-measured) · `FALLBACK_STALE` (a fabric surface returned a stale marker).

### A. Live fabric drive this session (MEASURED, local fabric :4949)
- `canon-index` LIVE: **631 md / 540 indexed / 92 orphans**.
- `council/query` **FIRED** `council-q-1782307690082-apckxu`, `by=acer`, **`sig_verdict=UNSIGNED`** — an ACK, **not** a signed seat verdict. (The `:4944` liris-mirror council returned `all_bases_unavailable`/cooldown last session → UNVERIFIED there. The local `:4949` council DID accept.)
- `loop/pending` = **17 envelopes, every one `auto_fire_allowed=false`** — the gate HOLDS; nothing auto-fired (consistent with prior sessions).
- `omni-control/status` LIVE: Falcon **SM-S721U1** (serial pinned), `LOOK/TYPE/ENTER` bridge, guardrail `no auto-dispatch`. → this **is** the operator's "remote-control agents inside the structure," MEASURED live.
- `supervisors` surface: **705 registered, by class/layer/sector**, fed from `D:/PID-Registration-Office` (substrate **PRESENT on disk**). → the Registration Office is a real registration substrate.

### B. Operator canon to preserve (cross-referenced to this census)
1. **omni systems = multi-level multi-fabric, ALL of them** `[CANON]`. Every `omni-*` row here (omnidispatcher, omniquant-host8, omnimets-collector, omni-control, omnispindle/omniflywheel) is itself multi-level and spans fabrics — not one daemon, not one level. The process-census rows are the *witnessed slice* of these (see §5 honest frame), not their full extent.
2. **A cube / structure does NOT carry a PID — it is *registered* at the REGISTRATION OFFICE in one of 113+ sectors** `[CANON]`. Reconciles with §5 Foundation-Invariant-4 (most seats are backend-shell-less function-call rotation = addressing/routing identities, **not** PID-bearing daemons) and with the live `supervisors` surface (705 registered, by **sector**, from `D:/PID-Registration-Office`). Registration = an address/sector slot, **distinct from minting a process PID**. Sector count **113+** is operator-stated `[UNVERIFIED exact]`; that sectors exist is `[MEASURED]`. This directly resolves §4's "per-PID harvest-mark join" framing: the class/layer seats are registered addresses, not missing processes.
3. **RECALL + HILBRA = the communication layer** bolted onto the addressing/nesting core `[CANON]`. `recall-serve` (rust-host8-done above) = local O(1) corpus search; **Hilbra** = cross-colony bidirectional search/comms ("system internet," broader than keyword search). Recall perf: `[MEASURED, prior session]` floor **~5,990 q/s** on the real **591,286-row** atlas (p50 10.36 ms) **while the box simultaneously ran** IIS + all Asolaria host + old json/node + MCP = a heavily-handicapped floor near the operator's world-record framing.
4. **N-PRIME infinite nesting + rule-of-4; possibly >16 levels now** `[CANON; UNVERIFIED exact count]`. The map previously assumed 16 levels — treat level-count as **open-ended**.
5. **Brown-Hilbert cubes + infinitely expandable 3D real-space + tensor collapse** `[CANON]`. Already partly present as `voxel_coord BH3D:x-y-z` in the Fischer port; the broader cube geometry + tensor collapse stay map surfaces (prior expansions are real, "has happened before").
6. **Construction-yard sector = primable for upgrade** `[OPERATOR_OBSERVED]` — added as an upgrade-candidate surface (new sector, not yet a process row).
7. **Remote-control agents + three agents nested infinitely** `[CANON]` — viable **"as long as the 8-byte host process + stubbed catalog cube folders persist."** The 3-agent nested pattern is the realization of N-prime/rule-of-4 nesting; liveness dependency = (a) 8-byte host process up + (b) stubbed catalog cube folders present. The remote-control lane itself is MEASURED live (omni-control, §A).
8. **Planned super-computer expansion + 35 TB Google Drive + NotebookLM staging** `[CANON / planned]` — tracked in `C:/tmp/acer-35tb/MIGRATION-PLAN.md` (LEG-4 `GoogleDriveStore`: built + offline round-trip green; cloud transport **GATED** on ADC `gcloud auth application-default login`). **NOT** landed in cloud yet `[UNVERIFIED cloud]`. "Drive" = **Google** Drive (never OneDrive).

### C. Bilateral state — GitHub is the mediator (2026-06-24)
- **LIRIS** pushed the Rust Fischer port to `asolaria-federation-1024` branch `liris/fischer-eval-host8-2026-06-24` (**PR #9**, commit `1522a30`) + public map artifacts to `HYPER-BECHS--the-third-set` (`liris` branch, `c8bfb9a`: Fischer receipt + `ASOLARIA-OMNI-HILBRA-ATLAS-EXPANSION-MAP-2026-06-24`).
- Liris seat **could not** run `cargo test` (`link.exe` absent) or `cargo fmt` (`rustfmt` absent) → **PR #9 is source-check parity only; the owning 1.81 CI was NOT satisfied** `[claims-gate: NOT "green/mergeable"]`.
- **ACER role:** `HOLD` on a competing Fischer push — acer's local `C:/tmp/host8-migration/fischer-eval-host8` staging stays local. Acer's additive value: it has the MSVC linker + 1.81/1.95 toolchains liris lacks → acer is the seat to run PR #9's **real owning gate** (1.81 `fmt`/`clippy -D`/`check --workspace`/no-bloat + `cargo test`) and post an honest attack-verify receipt. **Operator-gated.**
